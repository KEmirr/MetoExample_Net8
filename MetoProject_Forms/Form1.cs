...
namespace MetoProject_Forms
{
    public partial class Form1 : DevExpress.XtraEditors.XtraForm
    {
        ...
        private async Task LogToApiAsync(string logtype, string message, string source)
        {
            ...
            try
            {
                HttpResponseMessage response = await client.PostAsync("http://localhost:5000/api/log", content);
                if (response.IsSuccessStatusCode)
                {
                    memoEditLogs.Invoke(new Action(() =>
                    {
        ...
        {
            var response = await client.GetAsync("http://localhost:5000/api/PLCSettings");
        ...
        private async Task<bool> CheckPLCConnectAsync()
        {
            try
            {
                HttpResponseMessage Response = await client.GetAsync("http://localhost:5000/api/plc/check-connection");
                Response.EnsureSuccessStatusCode();
                string responseBody = await Response.Content.ReadAsStringAsync();
            ...
            {
                memoEditLogs.AppendText($"Request error PCLConFormsMessage: {ex.Message}\n");
                return false;
        ...
        private async Task WriteToPlcAsync()
        {
            try
            {
                HttpResponseMessage Response = await client.GetAsync("http://localhost:5000/api/plc/write");
                Response.EnsureSuccessStatusCode();
                string responseBody = await Response.Content.ReadAsStringAsync();
        ...
        private async Task<ushort?> ReadToPlcAsync()
        {
            try
            {
                HttpResponseMessage response = await client.GetAsync("http://localhost:5000/api/plc/read");
                response.EnsureSuccessStatusCode();
                string responseBody = await response.Content.ReadAsStringAsync();
